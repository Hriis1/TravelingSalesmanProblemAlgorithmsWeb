...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (($_POST["action"] ?? "") == "getUsersTsp") { //get users saved tsp
        $userId = (int) ($_SESSION["user_id"] ?? 0);
        $tspId = (int) ($_POST["tsp_id"] ?? 0);

        $res = $userService->getUsersTsp($userId, $tspId);
        echo json_encode($res);
        exit;
    }

    if (($_POST["action"] ?? "") == "getAllUsersTspsNames") { //get all users saved tsps ids and names
        $userId = (int) ($_SESSION["user_id"] ?? 0);

        $res = $userService->getAllUsersTspsNames($userId);
        echo json_encode($res);
        exit;
    }
